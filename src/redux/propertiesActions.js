...
export const fetchPropertiesArraySuccess = (propertiesArrayData) => ({
  type: FETCH_PROPERTYARRAYDATA_SUCCESS,
  payload: propertiesArrayData,
});

export const fetchPropertiesArrayFailure = () => ({
  type: FETCH_PROPERTYARRAYDATA_FAILURE,
});
...
export const fetchPropertiesArray = (axiosPrivate) => async (dispatch) => {
  ...
  try {
    ...
    if (status === 201 || status === 200) {
      console.log("propertyArrayssss",processedData)
      dispatch(fetchPropertiesArraySuccess(processedData));


      
    } else {
      dispatch(fetchPropertiesArrayFailure());
    }
  } catch (ex) {
    dispatch(fetchPropertiesArrayFailure());
    console.log({ex})
